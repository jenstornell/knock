...
class login {
  public static function loginUser() {
    ...
    header("Location: " . \knock\option('url'));
    die;
  }
  ...
  public static function isValidLogin() {
    ...
    if(!isset(\knock\option('users')[$username])) return false;
    if(\knock\option('users')[$username] != $password) return false;
  ...
  public static function createCookiesFolder() {
    $path = \knock\option('root') . '/cookies';
